...
def compute_motion_covariance(relative_pose: np.ndarray,
    ...
    delta_trans = np.sqrt((relative_pose[0]**2) + (relative_pose[1]**2))       # how far we translated
    delta_rot   = np.abs(relative_pose[2])                 # how much we rotated
    var_x     = alpha1 * delta_trans**2 + alpha2 * delta_rot**2
    var_y     = alpha1 * delta_trans**2 + alpha2 * delta_rot**2 # Squared to increase variance when doing large movements
    var_theta = alpha3 * delta_trans**2 + alpha4 * delta_rot**2
