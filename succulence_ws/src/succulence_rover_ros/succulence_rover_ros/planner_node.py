...
class PlannerNode(Node):
    ...
    def __init__(self):
        ...
        self.dilated_map_pub = self.create_publisher(OccupancyGridMsg, "/dilated_map",10)
    ...
    def _replan(self):
        ...
        # ADDED THIS MYSELF: Inflate obstacles for pathfinding but keep track of which cells are truly occupied so we can assign them a higher cost rather than blocking them outright.
        occupied = grid >= self.occ_threshold

        inflated = inflate_obstacles(
            grid,
            self.inflation_radius,
            self.occ_threshold,
            self.unknown_as_obstacle
        )
        ...
        debug_map = OccupancyGridMsg()
        debug_map.header.stamp = self.get_clock().now().to_msg()
        debug_map.header.frame_id = self.map_frame
        debug_map.info = info

        debug_data = np.where(inflated, 100, 0).astype(np.int8) #np.where(blocked, 100, 0).astype(np.int8)
        debug_map.data = debug_data.flatten().tolist()

        self.dilated_map_pub.publish(debug_map)
        ...
        path_cells = astar_search(blocked, start, goal, cost_map)
